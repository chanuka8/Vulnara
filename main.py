...
# command: insert the src directory to system path before importing local packages
...
def print_banner():
  ...
  |___/\__,_/_/_/ /_/\__,_/_/   \__,_/           
    ...
    """
    panel = Panel(Text(banner, style="bold cyan",
...
def print_findings_summary(result):
    console.print(
        f"[bold green][OK][/bold green] Findings generated: {len(result.findings)}")
    ...
    console.print(
        f"[bold green][OK][/bold green] Evidence saved: {result.evidence_path}")
    console.print(
        f"[bold green][OK][/bold green] Report saved: {result.report_path}")
...
def create_project(name: str, description: str):
    ...
    try:
        ...
        click.secho(
            f"[OK] Project created successfully: {proj.name} (ID: {proj.id})", fg="green")
...
def run_scan(target_url: str, domain: str, profile: str, ai: bool, project_name: str):
    ...
    if project_name:
        ...
        if not proj:
            click.secho(
                f"[ERROR] Project '{project_name}' not found. Create it first.", fg="red", err=True)
            sys.exit(1)
    else:
    ...
    discovery_engine = DiscoveryEngine(
        target_domain=domain, output_dir=discovery_dir)
    assets = discovery_engine.run()
    click.secho(
        f"[OK] Discovery completed. Found {len(assets)} assets. Saved to assets.json", fg="green")
    # -----------------------------------
...
@cli.command("tui")
def run_tui():
    # command: load the interactive textual interface component application
    from vulnara.ui.tui_app import VulnaraTUI
    app = VulnaraTUI()
    app.run()
